...
@IonicPage()
@Component({
  selector: 'page-scheduling-finalized',
  templateUrl: 'scheduling-finalized.html',
})
export class SchedulingFinalizedPage {
  ...
  getScheduling(showLoading = true) {
    ...
    this.http.post(this.web.api + "/scheduling-provider/list/", JSON.stringify({provider: user, status: 3}))
    .map(res => res.json())
    .subscribe(success => {
      if(showLoading){
        loading.dismiss();
